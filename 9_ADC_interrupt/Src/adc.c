...
//static void adc_cont_conv_init(void);//for pc0

uint32_t pc0_adc_read(void)
{
	return HAL_ADC_GetValue(&hadc1);//(hadc)
}
void adc_init_start(void)
{
	adc_cont_conv_init();
	HAL_ADC_Start(&hadc1);//(hadc)
}
...
void adc_DMA_init(void)
{
	...
	//config adc modle for ontinuous conversation
		...
		HAL_NVIC_SetPriority(DMA1_Channel1_IRQn,0,0);//(intrupt number(IRQn) PreemptPriority, SubPriority)
